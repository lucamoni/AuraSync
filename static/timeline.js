...
class AuraTimeline {
    ...
    renderSidebar() {
        const list = document.getElementById('timeline-function-list');
        if (!list) return;
        list.innerHTML = '';

        const appendType = (title, items, type) => {
            ...
            Object.entries(items).forEach(([id, fn]) => {
                const el = document.createElement('div');
                el.className = 'scene-badge draggable-func';
                el.innerText = fn.name || id;
                el.draggable = true;
                el.ondragstart = (e) => {
                    e.dataTransfer.setData('application/aurasync-function', JSON.stringify({
                        id, type, name: fn.name || id
                    }));
                };
                list.appendChild(el);
            });
        ...
        appendType('SCENE', this.functions.scenes, 'scene');
        appendType('CHASER', this.functions.chasers, 'chaser');
        appendType('EFX', this.functions.efx, 'efx');
    }
