...
/**
 * selectAddress — called when a DMX channel cell is clicked in the patch grid.
 * Highlights the cell and, if a fixture is selected, pre-fills the address field.
 */
function selectAddress(ch) {
    // Remove previous selection highlight
    document.querySelectorAll('.dmx-cell.selected').forEach(c => c.classList.remove('selected'));

    const cell = document.getElementById(`dmx-${ch}`);
    if (cell) cell.classList.add('selected');

    // If a fixture is in the inspector, update its address field for quick patching
    const addrInput = document.getElementById('inspect-address');
    if (addrInput && selectedFixture) {
        addrInput.value = ch;
        notify(`Indirizzo DMX selezionato: ${ch}`, 'info', 2500);
    } else if (!selectedFixture) {
        notify(`Canale ${ch} selezionato — scegli ora una fixture dalla libreria`, 'info', 2500);
    }
}
...
async function deleteFixture() {
    ...
    const confirmed = await confirmDialog(
        `Eliminare la fixture <strong>${selectedFixture.name}</strong> dal patch?`,
        'Elimina Fixture'
    );
    if (!confirmed) return;
    ...
    try {
        ...
        const data = await res.json();
        if (data.status === 'success') {
            notify(`Fixture "${selectedFixture.name}" eliminata`, 'success');
            ...
            notify('Errore eliminazione: ' + (data.message || 'Sconosciuto'), 'error');
        }
    } catch (e) {
        console.error(e);
        notify('Errore di rete durante eliminazione', 'error');
    }
    hideLoading();
}
        ...
        if (data.status === 'success') {
            ...
            for (const item of positions) {
                ...
                if (fix) {
                    ...
                    // Update 3D Visualizer immediately if instance exists
                    if (window.visualizer) {
                        window.visualizer.setFixturePosition(fix.id, item.position);
                    }
                }
            }

            notify(data.mock ? "Modalità MOCK (API Key mancante). Posizioni simulate." : "Analisi 3D completata e posizioni applicate!", "success");
            await loadActiveFixtures();

            // Seleziona la prima fixture per aggiornare l'inspector
            const firstId = positions.length > 0 ? positions[0].id : null;
            if (firstId) {
                selectFixtureInstance(firstId);
            }
        } else {
